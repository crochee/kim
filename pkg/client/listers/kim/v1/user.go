...
type UserLister interface {
	...
	List(selector labels.Selector) (ret []*kimv1.User, err error)
...
type userLister struct {
	listers.ResourceIndexer[*kimv1.User]
}

// NewUserLister returns a new UserLister.
func NewUserLister(indexer cache.Indexer) UserLister {
	return &userLister{listers.New[*kimv1.User](indexer, kimv1.Resource("user"))}
}

// Users returns an object that can list and get Users.
func (s *userLister) Users(namespace string) UserNamespaceLister {
	return userNamespaceLister{listers.NewNamespaced[*kimv1.User](s.ResourceIndexer, namespace)}
}
...
type UserNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*kimv1.User, error)
